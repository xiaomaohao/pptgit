...
`r chunk_reveal("flextable")`

```{r flextable, include=FALSE}

library(rio)
library(flextable)
student <- import("/Users/yangyongye/R/pptgit/sampledata/student_sample.dta")

score_summary <- student %>% 
  drop_na(w2chn, w2mat, w2eng) %>%
  get_summary_stats(w2chn,
                    w2mat, w2eng, type = "common")

score_summary %>%
  flextable(col_keys = c("variable","n",
                         "min","max","mean","sd")) %>%
  set_header_labels(variable="变量", n="数量",
                    min="最小值",max="最大值",mean="均值",sd="标准差") %>% 
